...
def login():
    ...
    st.title("Supply Chain & Finance Analytics Login")
    st.markdown("Please select your role and enter the password to continue.")

    role_selection = st.selectbox("Select your role:", ["Admin", "Track Accesser"])
    password = st.text_input("Password", type="password")

    if st.button("Login", type="primary"):
        try:
            admin_pass = st.secrets["passwords"]["admin"]
            tracker_pass = st.secrets["passwords"]["tracker"]

            if role_selection == "Admin" and password == admin_pass:
                st.session_state['authenticated'] = True
                st.session_state['role'] = "admin"
                st.rerun()
            elif role_selection == "Track Accesser" and password == tracker_pass:
                st.session_state['authenticated'] = True
                st.session_state['role'] = "tracker"
                st.rerun()
            else:
                st.error("❌ The password you entered is incorrect. Please try again.")
        except KeyError:
            st.error("Authentication secrets are not configured. Please contact the administrator.")
...
# If user is authenticated, show the main dashboard
    ...
    if data and file_timestamps:
        ...
        dd_numeric_cols = ['Qty in Cases', 'Qty in Ltrs']
        for col in dd_numeric_cols:
            if col in dd_df.columns:
                dd_df[col] = pd.to_numeric(
                    dd_df[col].astype(str).str.replace(r'[^\d.]', '', regex=True),
                    errors='coerce'
                ).fillna(0)
            else:
                dd_df[col] = 0
        ...
        dispatch_delivery_df['Dispatch Category'] = dispatch_delivery_df['Dispatch Days'].apply(categorize_days)
        dispatch_delivery_df['Delivery Category'] = dispatch_delivery_df['Delivery Days'].apply(categorize_days)
        
        if 'LR/Vehicle Num' in dispatch_delivery_df.columns:
            dispatch_delivery_df['Order Type'] = np.where(
                dispatch_delivery_df['LR/Vehicle Num'].str.contains('sales return', case=False, na=False),
                'Sales Return',
                'Regular Order'
            )
        else:
            dispatch_delivery_df['Order Type'] = 'Regular Order'
            st.warning("Warning: 'LR/Vehicle Num' column not found. Cannot identify sales returns.")
        ...
        st.sidebar.markdown(f"Welcome, **{st.session_state['role'].capitalize()}**!")
        ...
        if st.session_state['role'] == 'admin':
            ...
            page_options = [] # Fallback, should not happen
        ...
        # =================================================================================
            ...
            sales_return_master_df = dispatch_delivery_df[dispatch_delivery_df['Order Type'] == 'Sales Return']
            regular_orders_master_df = dispatch_delivery_df[dispatch_delivery_df['Order Type'] == 'Regular Order']

            if from_date_dd and to_date_dd:
                from_datetime_dd, to_datetime_dd = pd.to_datetime(from_date_dd), pd.to_datetime(to_date_dd)
                date_filtered_df = regular_orders_master_df[(regular_orders_master_df['DocDate'] >= from_datetime_dd) & (regular_orders_master_df['DocDate'] <= to_datetime_dd)]
                date_filtered_sales_returns = sales_return_master_df[(sales_return_master_df['DocDate'] >= from_datetime_dd) & (sales_return_master_df['DocDate'] <= to_datetime_dd)]
            else:
                date_filtered_df = regular_orders_master_df.copy()
                date_filtered_sales_returns = sales_return_master_df.copy()
            ...
            if depot_selection:
                filtered_df = date_filtered_df[date_filtered_df['WhsCode'].isin(depot_selection)]
                filtered_sales_returns_df = date_filtered_sales_returns[date_filtered_sales_returns['WhsCode'].isin(depot_selection)]
            else:
                filtered_df = date_filtered_df.copy()
                filtered_sales_returns_df = date_filtered_sales_returns.copy()
            
            if filtered_df.empty:
                st.warning("No regular order data available for the selected filters.")
            else:
                ...
                depot_summary = regular_orders_master_df.groupby('WhsCode').agg(
                ...
                if 'Qty in Cases' in regular_orders_master_df.columns and regular_orders_master_df['Qty in Cases'].sum() > 0:
                    depot_summary_cases_raw = regular_orders_master_df.groupby(['WhsCode', 'Billing Status'])['Qty in Cases'].sum().unstack(fill_value=0)
            ...
            st.divider()
            with st.expander(f"Sales Return Orders ({len(filtered_sales_returns_df)})"):
                st.write("The following orders were identified as 'Sales Return' based on the 'LR/Vehicle Num' column and are excluded from the performance metrics above.")
                if not filtered_sales_returns_df.empty:
                    st.dataframe(filtered_sales_returns_df)
                    csv_sales_return = convert_df_to_csv(filtered_sales_returns_df)
                    st.download_button(
                        label="Download Sales Return Data as CSV",
                        data=csv_sales_return,
                        file_name="sales_return_data.csv",
                        mime="text/csv"
                    )
                else:
                    st.info("No sales return orders found for the current filter selection.")

            st.subheader("Download Detailed Logistics Data (Regular Orders)")
        ...
        # --- PAGE 6: Tracking ---
        # =================================================================================
        elif page == "Invoice Tracking":
            display_last_refreshed(['dbmaster', 'dd', 'depot_mapping'], file_timestamps)
            st.header("Track Order Details")
            st.sidebar.info("Enter a Invoice Number on this page to see its specific details.")
            
            doc_num_input = st.text_input("Enter the Invoice Number (DocNum) to track:", placeholder="Type InvNum and press Enter")

            if doc_num_input:
                tracking_df = dispatch_delivery_df[dispatch_delivery_df['DocNum'].astype(str) == str(doc_num_input)]

                if not tracking_df.empty:
                    st.success(f"Found details for Document Number: **{doc_num_input}**")
                    details = tracking_df.iloc[0]
                    st.markdown("---")
                    ...
                    with col1:
                        ...
                        st.markdown(f"**Invoice Number:** `{details.get('DocNum', 'N/A')}`")
                        doc_date = pd.to_datetime(details.get('DocDate'))
                        st.markdown(f"**Invoice Date:** {doc_date.strftime('%d-%b-%Y') if pd.notna(doc_date) else 'N/A'}")
                        st.markdown(f"**Quantity in Cases:** {details.get('Qty in Cases', 0):,.2f}")
                        st.markdown(f"**Quantity in Liters:** {details.get('Qty in Ltrs', 0):,.2f}")
                    ...
                    with col3:
                        ...
                        dispatch_date = pd.to_datetime(details.get('LR/Dispatch Date'))
                        delivery_date = pd.to_datetime(details.get('U_Delivery_Date'))
                        st.markdown(f"**Dispatch Date:** {dispatch_date.strftime('%d-%b-%Y') if pd.notna(dispatch_date) else 'Not Dispatched Yet'}")
                        st.markdown(f"**Delivery Date:** {delivery_date.strftime('%d-%b-%Y') if pd.notna(delivery_date) else 'Not Delivered Yet'}")
                        st.markdown(f"**Dispatch Timeline (Days):** `{details.get('Dispatch Days', 'N/A')}`")
                        st.markdown(f"**Delivery Timeline (Days):** `{details.get('Delivery Days', 'N/A')}`")

                    st.markdown("---")

                else:
                    st.error(f"No order found with Inv Number: **{doc_num_input}**. Please check the number and try again.")
            else:
                st.info("Please enter a Inv Number in the box above to track an order.")
